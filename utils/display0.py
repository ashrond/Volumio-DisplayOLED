...
def animate_gif(device, overlay_callback=None):
    """Displays an animated GIF with optional UI overlay elements."""
...
def scroll_text(device, title, speed=2):
    ...
    for offset in range(scroll_range):
        with canvas(device) as draw:
            draw_static_info(draw, title_offset=offset)
